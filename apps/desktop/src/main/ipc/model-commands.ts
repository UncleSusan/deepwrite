...
export async function handleModelCommands(
  ...
  command: CommandEnvelope
      ...
      if (command.type === "models.list") {
        try {
          return {
            ...
            payload: ModelSettingsSchema.parse(await ctx.requireModelConfigStore().list())
          };
        } catch (error: unknown) {
          return {
            status: "rejected",
      ...
      if (command.type === "models.saveOfficialToken") {
        try {
          const settings = ModelSettingsSchema.parse(
            await ctx.requireModelConfigStore().saveOfficialToken(command.payload.apiKey)
          );
          await ctx.requireModelUsageStore().syncConfiguredModels(settings.models);
          return {
            status: "accepted",
      ...
      if (command.type === "models.setOfficialModelEnabled") {
        try {
          const settings = ModelSettingsSchema.parse(
            await ctx.requireModelConfigStore().setOfficialModelEnabled(
      ...
      if (command.type === "models.save") {
        try {
          ...
          void ctx.requireModelUsageStore()
      ...
      if (command.type === "models.test") {
        try {
          const runtimeConfig = await ctx.requireModelConfigStore().resolveDraft(
          ...
          const result = await ctx.supervisor.requestCommand("agent", internalCommand, 20_000);
