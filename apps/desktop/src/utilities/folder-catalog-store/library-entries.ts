...
export async function saveLibraryEntry(
  store: FolderCatalogStoreContext,
    ...
    assertTextByteLength(input.content, store.maxMarkdownBytes, "Markdown content");
...
export async function moveLibraryEntry(
  store: FolderCatalogStoreContext,
    ...
    return await mutate(store, async () => {
      ...
      const sourceManifest = await readManifest(store,
        sourceDirectory,
      ...
      if (input.sourceLibraryId === input.targetLibraryId) {
        ...
        await atomicWriteJson(join(sourceDirectory, MANIFEST_FILE), next, store.maxManifestBytes);
        await bumpRegistry(store, registry, now);
      ...
      const targetManifest = await readManifest(store,
        targetDirectory,
        input.domain === "material" ? "deepwrite.material-library" : "deepwrite.skill-library",
      ...
      const sourceContent = await readProjectMarkdown(sourceDirectory, sourceEntry.path, store.maxMarkdownBytes);
      ...
      await commitProjectMarkdownUpdate(targetFile, sourceContent, undefined, join(targetDirectory, MANIFEST_FILE), nextTarget, store.maxMarkdownBytes, store.maxManifestBytes);
      await atomicWriteJson(join(sourceDirectory, MANIFEST_FILE), nextSource, store.maxManifestBytes);
