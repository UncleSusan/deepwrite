...
export async function createBookProject<Resource extends Book>(
  store: FolderCatalogStoreContext,
    ...
    return await mutate(store, async () => {
      ...
      const projectDirectory = await writeNewResourceProject(store,
        "book",
        parentDirectory,
        book
      );
      ...
      return (await readProject(store,
        projectDirectory,
        "book"
...
export async function updateLibrary(
  store: FolderCatalogStoreContext,rawInput: UpdateLibraryInput): Promise<MaterialLibrary | SkillLibrary> {
    ...
    return await mutate(store, async () => {
      ...
      await atomicWriteJson(join(projectDirectory, MANIFEST_FILE), next, store.maxManifestBytes);
      await bumpRegistry(store, registry, now);
      return (await readProject(store, projectDirectory, libraryProjectDomain(input.domain))).resource as MaterialLibrary | SkillLibrary;
    });
  }
...
export async function createLibraryGroup(
  store: FolderCatalogStoreContext,
    ...
    const parentDirectory =
      rawInput.parentDirectory?.trim() || store.defaultProjectParents[projectDomain];
    return await mutate(store, async () => {
      const registry = await ensureRegistry(store);
      const snapshot = await aggregateSnapshot(store, registry);
...
export async function openBookProject(
  store: FolderCatalogStoreContext,
    ...
    return (await openCatalogProject(store,
      projectDirectory,
      "book",
      register
...
export async function updateBook(
  store: FolderCatalogStoreContext,rawInput: UpdateFolderBookInput): Promise<Book> {
    ...
    return await mutate(store, async () => {
      ...
      const opened = await readProject(store,
      ...
      await bumpRegistry(store, registry, now);
      return (await readProject(store, opened.projectDirectory, "book")).resource as Book;
    });
  }
...
export async function removeBook(
  store: FolderCatalogStoreContext,bookId: string): Promise<{ bookId: string; deleted: boolean }> {
