...
describe("LongProjectStore: commit-recovery-and-cas", () => {
  ...
  it("writes the chapter triplet atomically, commits continuity, and rolls back only the last commit", async () => {
    ...
    const written = await projectStore.writeChapter(created.projectDirectory, {
      chapterCardId,
      body: {
        content: "雨夜里，她收到一封信。",
        baseRevision: files.body.revision
    ...
    expect(committed.record).toMatchObject({
      ...
      placementChanges: [{ note: "正文明确写出林岚收到旧信。" }],
      foreshadowingBeatChanges: [{ note: "正文展示旧信与寄信人身份线索。" }],
    ...
    expect(afterCommit.book.workspaceIndex.plot.foreshadowing[0]!.status).toBe(
      "open"
    );
    expect(afterCommit.book.workspaceIndex.ledger.projection).toMatchObject({
      ...
      openLoops: [{ loopId: "loop_old-postmark", status: "open" }],
      latestHandoff: {
    ...
    await expect(
      ...
      content: expect.stringContaining("## 未闭合事项\n\n- loop_old-postmark")
    });
    for (const file of [
    ...
    await expect(
      projectStore.writeDocument(created.projectDirectory, {
        ...
        content: "回滚后可以继续修改正文。",
        expectedFileRevision: firstChapterFiles(afterRollback.book).body
          .revision,
  ...
  it("fails closed before extending a ledger with any tampered pinned file", async () => {
    const { projectStore, created } = await createFixture("pinned-integrity");
    const emptyRevision = createLongFileRevision("");
    ...
    const characterStorage = projectTransactionContentSha256(characterId).slice(
      0,
      32
    );
    const characterFiles = {
    ...
    const secondCommitInput: Parameters<LongProjectStore["commitChapter"]>[1] =
      {
        chapterCardId: secondChapterId,
        chapterFileRevisions: {
    ...
    const tamperCases = [
      ...
      {
        path: join(created.projectDirectory, characterFiles.relationships.path),
        content: "篡改账本接管的人物关系"
      }
    ];
    ...
    for (const tamperCase of tamperCases) {
      ...
      await expect(
        projectStore.commitChapter(created.projectDirectory, secondCommitInput)
      ).rejects.toThrow();
    ...
    await expect(
      racingStore.commitChapter(created.projectDirectory, secondCommitInput)
