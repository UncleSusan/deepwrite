...
import type { LibraryAgentDomain, ThinkingLevel, UserPromptAttachment } from "@deepwrite/contracts";
import type { AgentApprovalMode, ComposerReferenceOption, EditorTextReference } from "../types/conversation";
import type { IconName } from "../types/workspace";
import { PROMPT_ATTACHMENT_ACCEPT } from "../utils/promptAttachments";
import { useConversationAttachments, attachmentPreview, formatFileSize } from "../composables/useConversationAttachments";
...
const props = defineProps<{
  ...
  approvalOptions: Array<{ value: AgentApprovalMode; label: string; description: string }>;
...
const {
  ...
  handleAttachmentChange, handleComposerPaste, removePendingAttachment
...
const {
  ...
  updateActiveReference, handleInput, closeReferenceMenu, selectReference, submitMessage,
  handleKeydown
...
function editorReferenceTooltip(reference: EditorTextReference): string {
  const preview = reference.text.length > 1_000 ? `${reference.text.slice(0, 1_000)}…` : reference.text;
  return `${reference.documentPath.join(" / ")}\n第 ${reference.startLine}-${reference.endLine} 行\n\n${preview}`;
}
function handleModelChange(value: string | number): void { emit("selectModel", String(value)); }
function handleThinkingChange(value: string | number): void { emit("selectThinking", String(value) as ThinkingLevel); }
function handleTemperatureChange(value: string | number): void { emit("selectTemperature", Number(value)); }
function handleApprovalChange(value: string | number): void {
  if (value === "request-approval" || value === "auto-approve") emit("selectApproval", value);
}
</script>
