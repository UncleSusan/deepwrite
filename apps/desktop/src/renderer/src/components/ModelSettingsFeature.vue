...
function cloneTemperatureOptions(
  options: TemperatureOptions
): TemperatureOptions {
  return [options[0], options[1], options[2]];
}

function cloneThinkingLevelOptions(
    ...
    if (props.active && !saving) {
      ...
      if (editor?.managedBy === "deepwrite-free" && settings) {
        draftModels.value = draftModels.value.map((model) => {
          ...
          const refreshed = settings.models.find(
...
function applyDeepWriteFreeModel(modelId: string): void {
  const editor = modelEditor.value;
  const preset = deepwriteFreeModels.value.find(
...
function setToolSchemaProfile(value: string | number): void {
  ...
  if (profile === "auto") {
    delete modelEditor.value.toolSchemaProfile;
    return;
  }
  modelEditor.value.toolSchemaProfile = profile as ToolSchemaProfile;
}
...
function toggleThinkingLevelOption(
  ...
  if (
    editor.reasoning &&
    !editor.thinkingLevelOptions.includes(
      editor.defaultThinkingLevel as ReasoningLevel
    )
  ) {
...
function updateCustomThinkingLevel(event: Event): void {
  ...
  editor.thinkingLevelOptions = editor.thinkingLevelOptions.filter(
...
function saveModelEditor(): void {
  ...
  if (
    ...
    (editor.temperatureOptions.some(
      ...
      new Set(editor.temperatureOptions).size !==
        editor.temperatureOptions.length)
  ) {
  ...
  const normalized: DraftModel = {
    ...
    defaultThinkingLevel: editor.reasoning
      ? editor.defaultThinkingLevel
      : "off",
    thinkingLevelOptions: cloneThinkingLevelOptions(
      editor.thinkingLevelOptions
    ),
  ...
  const duplicateIndex = draftModels.value.findIndex(
    (model, candidateIndex) =>
      model.id === normalized.id && candidateIndex !== index
  );
  if (duplicateIndex >= 0) {
            ...
            >
              <div class="model-editor-heading">
                <strong>{{
                  draftModels.some(
                    (model) =>
                      model.id === (modelEditor?.originalId ?? modelEditor?.id)
                  )
                    ? "编辑模型"
                    : "添加模型"
                }}</strong>
              ...
              <div class="model-form-grid">
                ...
                <label>
                  ...
                  <PopupSelect
                    ...
                    :menu-min-width="300"
                    @update:model-value="
                      applyDeepWriteFreeModel(String($event))
                    "
                  />
                  <div v-else class="model-id-field">
                    ...
                    <button
                      ...
                      "
                      :aria-label="
                        listingRemoteModels ? '拉取中' : '拉取可用模型'
                      "
                ...
                <label v-if="!isDeepWriteFreeEditor">
                  ...
                  <PopupSelect
                    :model-value="
                      modelEditor.reasoning ? 'reasoning' : 'temperature'
                    "
                    :options="modelModeOptions"
                    accessible-label="选择模型模式"
                    @update:model-value="
                      setModelMode(
                        String($event) as 'reasoning' | 'temperature'
                      )
                    "
                  />
                </label>
                ...
                <label
                  v-if="!isDeepWriteFreeEditor && modelEditor.reasoning"
                  class="is-wide"
                >
                  <span>思考等级选项</span>
                  <span class="model-thinking-options">
                    ...
                    >
                      <input
                        type="checkbox"
                        :checked="
                          modelEditor.thinkingLevelOptions.includes(
                            option.value
                          )
                        "
                        @change="
                          toggleThinkingLevelOption(option.value, $event)
                        "
                      />
                    ...
                    <span
                      ...
                      :title="
                        modelEditor.customThinkingLevel?.trim() || 'custom'
                      "
                      :data-tooltip="
                        modelEditor.customThinkingLevel?.trim() || 'custom'
                      "
                ...
                <p
                  v-if="isDeepWriteFreeEditor"
                  class="model-managed-note is-wide"
                >
                  模型名称和参数由 DeepWrite
                  远程配置自动维护；运行环境提供密钥，无需在此填写。
                </p>
              </div>
              <div
              ...
              >
                ...
                <button
                  type="button"
                  @click="
                    modelEditor.hasApiKey = false;
                    modelEditor.clearApiKey = true;
                    modelEditor.apiKey = '';
                  "
                >
                  清除已保存密钥
            ...
            >
              ...
              <div>
                ...
                <small
                  ...
                  · {{ row.model.modelId }} · {{ row.model.api }}</small
                >
                <small>
                  {{
                    row.model.reasoning
              ...
              <div class="model-card-actions">
                ...
                <button
                  v-if="row.model.managedBy !== 'deepwrite-official'"
                  type="button"
