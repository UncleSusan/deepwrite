...
describe("DeepWrite Pi runtime adapter: provider-model-routing", () => {
  ...
  it("publishes literal string unions as enums to Anthropic-compatible models", async () => {
      const config: AgentProviderRuntimeConfig = {
        id: "anthropic-schema",
      ...
      expect(parameters.properties.kind).toHaveProperty("anyOf");
      expect(parameters.properties.kind).not.toHaveProperty("enum");
    });
