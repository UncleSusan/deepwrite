...
describe("long workspace agent tools: chapter-queries-and-mutations", () => {
  ...
  it("forms a dispatch proposal from one index query and stops cleanly when complete", async () => {
    ...
    const completeExecutor = vi.fn<LongCommandExecutor>(async (command) => ({
      status: "accepted",
      requestId: command.id,
      payload: {
        bookId: "longbook_tools",
  ...
  it("rejects a Core worldbuilding response that changes the internally mapped file", async () => {
    ...
    await expect(
      toolByName(tools, "read_setting").execute("read-wrong-file", {
  ...
  it("creates exactly one empty worldbuilding file and returns its stable item id", async () => {
    ...
    const proposal = await toolByName(tools, "create_setting").execute(
      "create-world-items",
      {
        domain: "worldbuilding",
        category_id: "world_rules",
    ...
    if (proposal.details?.kind !== "long-worldbuilding-file-proposal") {
  ...
  it("returns independent long worldbuilding file proposals for full writes and edits", async () => {
    ...
    await toolByName(tools, "read_setting").execute("read-world-file", {
      domain: "worldbuilding",
    ...
    const edit = await toolByName(tools, "edit_setting").execute(
      "edit-world-file",
      {
        domain: "worldbuilding",
        category_id: category.id,
