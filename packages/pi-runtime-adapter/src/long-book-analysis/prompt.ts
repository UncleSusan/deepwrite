...
function phaseRequirements(
  ...
  inputMode: "tools" | "inline"
): string {
  const textOutput = outputMode === "text";
  const inlineInput = inputMode === "inline";
  if (context.phase === "batch") {
    return [
      "当前是分批分析阶段。",
      inlineInput
        ? "当前批次的章节片段已直接附在用户指令中，必须完整依据这些内容分析，且不得调用工具。"
  ...
  if (context.phase === "reduce") {
    return [
      "当前是中间笔记归并阶段。",
      inlineInput
        ? "当前轮全部输入笔记已直接附在用户指令中，必须完整依据这些内容归并，且不得调用工具。"
  ...
  return [
    "当前是最终结果生成阶段。",
    inlineInput
      ? "当前轮归并笔记已直接附在用户指令中，必须完整依据这些内容生成结果，且不得调用工具。"
...
export function renderLongBookAnalysisSystemPrompt(
  ...
  return [
    ...
    `预设：${profile.name}`,
    phaseRequirements(context, outputMode, inputMode),
