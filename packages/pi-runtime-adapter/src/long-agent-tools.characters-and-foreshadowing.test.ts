import type { LongCommandExecutor } from "./long-agent-tools.test-support";
import {
  Check,
...
describe("long workspace agent tools: characters-and-foreshadowing", () => {
  it("lists and creates characters with a custom dynamic type id", async () => {
    ...
    expect(
      Check(toolByName(tools, "create_setting").parameters, {
        domain: "character",
        name: "陆烬",
    ...
    const listedText = resultText(
      await toolByName(tools, "list_setting").execute("list-custom-types", {
        domain: "character",
        type_id: "chartype_antagonist"
      })
    );
    ...
    expect(
      JSON.stringify(toolByName(tools, "list_setting").parameters)
    ).not.toContain('"page"');
  ...
  it("requires a full character-document read before safe replacement", async () => {
    ...
    const beforeRead = await toolByName(tools, "edit_setting").execute(
      "replace-before-read",
      {
        domain: "character",
    ...
    await toolByName(tools, "read_setting").execute("read-character", {
      domain: "character",
      character_id: "character_alice",
      document: "core_profile",
      mode: "full"
    });
    const proposal = await toolByName(tools, "edit_setting").execute(
      "replace-after-read",
      {
        domain: "character",
        character_id: "character_alice",
  ...
  it("creates character files and chapter triplets with runtime-owned ids and empty content revisions", async () => {
    ...
    expect(plotProfile.writeAccess.workspaceRoots).toEqual(["plot_design"]);
  ...
  it("translates foreshadowing planning fields while requiring canonical operation names", async () => {
    const executor = vi.fn<LongCommandExecutor>(async (command) => {
      if (command.type !== "long.getWorkspaceIndex") {
        throw new Error(`Unexpected command: ${command.type}`);
      }
    ...
    expect(preparedAliasCall).toBeUndefined();
    expect(
    ...
    const proposal = await mutationTool.execute("foreshadowing-planning", {
      domain: "worldbuilding",
      operations: [
        {
          type: "volume.create",
  ...
  it("computes document revisions from logical targets and rejects the generic draft-write bypass", async () => {
    ...
    expect(
      JSON.stringify(toolByName(plotTools, "propose_long_mutation").parameters)
    ).not.toContain('"kind":"draft"');
    await expect(
      toolByName(plotTools, "propose_long_mutation").execute("draft-bypass", {
        domain: "worldbuilding",
        operations: [
          {
            type: "chapter.update",
  ...
  it("locks chapter writes to the active chapter and accepts body content only", async () => {
    ...
    const result = await toolByName(tools, "write_chapter_draft").execute(
      "chapter-write",
      writeInput
    );
  ...
  it("requires a full read and explicit permission before overwriting non-empty chapter text", async () => {
    ...
    expect(
      resultText(await write.execute("without-confirmation", input))
