...
describe("long workspace agent tools: capabilities-and-plot-reading", () => {
  it("assembles exact query and proposal tools by long capability only", () => {
    ...
    forgedSettingProfile.writeAccess.capabilities.push("write_chapter_files");
    ...
    expect(rootlessLedgerNames).not.toContain("propose_continuity_commit");
  });
  ...
  it("rejects a cross-volume chapter move before creating an approval proposal", async () => {
    ...
    const result = await toolByName(tools, "propose_long_mutation").execute(
      "cross-volume-chapter-move",
      {
        operations: [
          {
            type: "chapter.move",
    ...
    const createResult = await toolByName(tools, "create_plot_design").execute(
      "cross-volume-chapter-create",
      {
  ...
  it("loads only long-bound resources allowed by the active long profile", async () => {
    ...
    const skill = await toolByName(tools, "load_skill").execute("load-skill", {
      name: "规则一致性"
    });
  ...
  it("lists plot design as paragraph text without pagination", async () => {
    ...
    expect(
      Check(list.parameters, { kind: "arc", volume_id: "volume_one" })
    ).toBe(true);
    expect(Check(list.parameters, { kind: "chapter_card" })).toBe(false);
    expect(preparedChapterAlias).toBeUndefined();
    expect(preparedChapterTarget).toBeUndefined();
