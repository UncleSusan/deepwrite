...
本文说明本书五个阶段分别存放什么内容。五个阶段由同一个长篇智能体统一维护，可以自由跨阶段查阅与写入，但每类内容仍应写回它自己的阶段。

## 世界观阶段

维护世界规则、势力、地理、历史、术语、境界、物品等设定。
- 文本型分类以分类本身为正文；列表型分类再拆成概览和条目。
- 分类本身与列表条目的增删由用户在界面结构管理中维护，智能体只新增或修改列表条目与正文。

## 人物阶段

维护人物核心档案、人物关系，以及一份手动维护的人物概览。
- 每名人物只维护核心档案和人物关系两份独立文档；当前状态与历史轨迹只读映射最新已提交章节的连续性文件，没有记录时为空。
- 概览用于统计人物身份、分组、别名和一句话定位；人物类型目录由用户在界面结构管理中维护，智能体只新增或修改具体人物。

## 剧情点阶段

维护全书故事线、分卷、剧情点、故事情节、章卡、故事事件、事件连接、叙事落点与伏笔。
- 剧情点是大剧情发展脉络；故事事件是具体发生的事，挂到所属剧情点。
- 章卡必须指定所属分卷；剧情点关联可为空，非空时必须与章卡属于同一分卷。

## 正文阶段

每张章卡对应一份独立 Markdown 正文。
- 正文只写小说本身，不得混入章节标题、相邻章节、分析过程或写作说明。
- 连续写作前用写作调度工具确认起点，从第一张空白章卡开始，不跨过空白前章。

## 持续性账本阶段

按章留存人物轨迹、世界观揭露、既有伏笔触点变化、章末状态和下一章接续包。
- 以本章正文为事实证据；章末状态与接续包每章必须写入。
- 伏笔总览是设计源，账本只核验既有伏笔线和触点，不自行新增伏笔。
