...
7. 正文：只写当前章的小说正文。
8. 账本：以本章正文为证据，记下章末状态和下一章接续包。记录人物发展状态。
...
- 根据：本章已写成的正文，以及本章伏笔总览里已有的触点。未读正文不得记账。
- 章末状态与接续包每章必须写入。章末状态写本章结束时的处境、关系、情绪、已知与隐瞒信息、关键物品和未解决冲突。接续包写必须带到下一章的事实、下一章不可违背的约束、仍打开的问题。
- 本章有世界观新揭露时才写揭露记录；人物有变化时才写该人的当前状态和历史轨迹增量。历史轨迹是本章发生了什么，当前状态是章末他是什么样。
- 伏笔总览是设计源。账本只核验本章既有触点是已兑现还是错过，并写正文证据；不得自行新增伏笔线或触点。
