...
edit supports both whole-document writes and local edits. If the target is empty, write it directly with content. Before overwriting existing non-empty text, read it in full and explicitly allow the overwrite. For a local edit, use replacements to replace a unique original passage from the fully read text. edit may also include meta to change a title or relationship field. delete removes one specific object. If downstream references exist, the tool will also report the cascading impact for the user to confirm on the approval card.
