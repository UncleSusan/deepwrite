...
function uniqueIdArray<T extends string>(schema: z.ZodType<T>, label: string) {
  return z
    .array(schema)
    .max(400_000)
...
function orderedChapterIds(workspace: LongWorkspaceIndexSnapshot): string[] {
  const volumes = volumeOrderMap(workspace);
...
function deleteForeshadowingBeat(state: MutationState, beatId: string): void {
    ...
    case "worldbuilding.update": {
      ...
      if (
        ...
        if (category.format === "list") {
          ...
          const itemId = `worlditem_${category.id.replace(/^world_/u, "").slice(0, 120)}:converted`;
